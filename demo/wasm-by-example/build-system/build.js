...
const mustacheData = {
    ...
    js: {
        index: `const WASM_BY_EXAMPLE_VERSION = "${
            packageJson.version
        }";${minifyJs("shell/js/index.js")}`,
...
const buildTask = async () => {
    ...
    mustacheData.examples.forEach(example => {
        ...
        let index = mustacheData.examplesByLanguage.findIndex(
            exampleByLanguage => {
                if (
    ...
    homepage.categories.forEach((category, index) => {
        ...
        mustacheData.categories[index].title.inLanguage =
            category.title["en-us"];
    ...
    homepage.readingLanguages.forEach(readingLanguage => {
        ...
        homepageMustacheData.categories.forEach(category => {
            ...
            category.description.inLanguage =
                category.description[readingLanguage] ||
                category.description["en-us"];
        });

        const homeContents = fs
            .readFileSync(`shell/home.html`, "utf8")
            .toString();
        const renderedPage = Mustache.render(
            homeContents,
            homepageMustacheData
        );
